...
class policy_value_net(nn.Module):
    def __init__(self, block, inplanes, planes, stride=1):
        ...
        # 价值头
        self.conv2 = nn.Conv2d(36, 4, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(4)
        self.fc1 = nn.Linear(196, 72)
        self.fc2 = nn.Linear(72, 1)
        # 策略头
        self.conv3 = nn.Conv2d(36, 2, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.bn3 = nn.BatchNorm2d(2)
        self.fc3 = nn.Linear(98, 84)
    ...
    def forward(self,x):
        ...
        value_out = value_out.view(-1, 196)
        ...
        policy_out = policy_out.view(-1, 98)
...
class PolicyValueNet(object):
    ...
    def __init__(self,model_file=None, use_gpu=True):
        ...
        if self.use_gpu:
            ...
            self.policy_value_net = policy_value_net(BasicBlock,26,36).cuda()
        else:
            # device = torch.device("cpu")
            self.policy_value_net = policy_value_net(BasicBlock,26,36)
    ...
    def policy_value_fn(self, game):
        ...
        current_state = np.ascontiguousarray(game.state()).reshape([1,26,7,7])
        if self.use_gpu:
            # device = torch.device("cuda:0")
